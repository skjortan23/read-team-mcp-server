...
class Scanner:
    ...
    async def validate_masscan(self) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                logger.error(f"Masscan validation failed: {stderr.decode()}")
                return False
            
            version_info = stdout.decode().strip()
            logger.info(f"Masscan validation successful: {version_info}")
            return True
    ...
    def _build_masscan_command(self, request: ScanRequest, output_file: Path) -> List[str]:
        ...
        if request.scan_type == "tcp_syn":
            cmd.append("--tcp-syn")
        elif request.scan_type == "tcp_connect":
